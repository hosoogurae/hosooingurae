...
function findFeatureLineCandidate(text: string): FeatureLineCandidate | undefined {
  ...
  for (const line of lines) {
    ...
    // (1) ㎡·괄호·슬래시·% 같은 단위/기호가 있거나, (2) 숫자와 함께 쓰인
    // "원"으로 끝나는 실제 금액 표현("25만원", "500원")이거나, (3) 토큰
    // 전체가 숫자(+선택적 단위 접미사)로만 이뤄진 경우(가격·면적 그
    // 자체)만 걸러내고, 한글 단어 속에 숫자가 섞인 토큰은 그대로
    // 통과시킵니다.
    //
    // "원" 글자 자체를 금지하면 "공원"/"학원"/"정원"/"병원"처럼 원래
    // "원"으로 끝나는 흔한 한글 단어까지 통화 단위로 오인해 걸러지므로
    // (실제 사례: "공원뷰굿"), 숫자가 붙어 있을 때만 금액으로 판단합니다.
    const hasBadToken = tokens.some(
      (token) =>
        /[㎡()/%％]/.test(token) ||
        (/\d/.test(token) && /원$/.test(token)) ||
